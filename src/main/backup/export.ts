...
function exportZavodPayload(db: DatabaseSync, zavodId: number): BackupZavodPayload {
  const zavod = db
    .prepare('SELECT id, nazev, datum, misto, typ FROM zavod WHERE id = ?')
    .get(zavodId) as unknown as BackupZavodRow | undefined
  ...
  const kategorie = db
    ...
    .all(zavodId) as unknown as BackupKategorieRow[]
  ...
  const skupiny = db
    ...
    .all() as unknown as BackupSkupinaRow[]
  ...
  if (koloIds.length > 0) {
    jizdy = db
      ...
      .all() as unknown as BackupJizdaRow[]
  }
  ...
  if (jizdaIds.length > 0) {
    ...
    rost_pozice = db
      ...
      .all() as unknown as BackupRostPoziceRow[]
    vysledky = db
      .prepare(
  ...
  if (vysIds.length > 0) {
    uprava_log = db
      ...
      .all() as unknown as BackupUpravaLogRow[]
  }
...
function exportGlobalTables(db: DatabaseSync): {
  ...
  const zebricek = db
    ...
    .all() as unknown as BackupZebricekRow[]
  const pravidla = db
    .prepare(
    ...
    .all() as unknown as BackupPravidlaRow[]
  const nastaveni = db
    .prepare('SELECT klic, hodnota FROM nastaveni ORDER BY klic')
    .all() as unknown as BackupNastaveniRow[]
